...
class BrownLensDistortion(LensDistortion):
    ...
    def my_distortImage(self, image, scale=None, K=None):
        height = image.shape[1]
        width = image.shape[2]
        i, j = np.meshgrid(
            np.linspace(0, width, int(width/20)),
            np.linspace(0, height, int(height/20)),
            indexing="ij",
        )
        i = i.T
        j = j.T
        ...
        r = torch.sqrt(torch.sum(flow**2, dim=-1, keepdim=True))
        radial = 1.
        #radial = (1 + self.k1*r**2 + self.k2*r**4 + self.k3*r**6)
        #theta = torch.atan(r)
        #radial = theta / r

        x = flow[..., 0].cpu().numpy()
        y = flow[..., 1].cpu().numpy()
        plt.figure(figsize=(width/80, height/80))  # Set the figure size
        plt.scatter(x, y, s=1)  # 's' is the size of each point
        plt.title('Scatter Plot of 2D Points')
        plt.xlabel('X coordinate')
        plt.ylabel('Y coordinate')
        plt.savefig('./before_flow.jpg', format='jpg')
        ...
        plt.xlabel('X coordinate')
        plt.ylabel('Y coordinate')
        plt.savefig('./after_flow.jpg', format='jpg')
        plt.close()  # Close the plot to free up memory

        flow[:, :, 0] = flow[:, :, 0] * scale[0]
        flow[:, :, 1] = flow[:, :, 1] * scale[1]
        ...
        plt.xlabel('X coordinate')
        plt.ylabel('Y coordinate')
        plt.savefig('./after_K.jpg', format='jpg')
        plt.close()  # Close the plot to free up memory
        import pdb;pdb.set_trace()

        distort_image = F.grid_sample(image.cuda().float().unsqueeze(0), flow.cuda().float().unsqueeze(0), align_corners=False, padding_mode="border")[0]

        distort_image = distort_image.permute(1,2,0)
        return distort_image

    def distortImage(self, image, scale=None):
        x, y = torch.meshgrid([torch.arange(0, self.image_width_px),
                    torch.arange(0, self.image_height_px)], indexing='ij')
        x = x.float() + 0.5
        y = y.float() + 0.5
